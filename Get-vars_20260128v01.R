...
### 5) Repeat process for mother relevant variables for step 1.

#### 1) Participant type

AAB_data_recoded <- AAB_data_recoded %>%
  mutate(
    participant_type = na_if(participant_type, 999),
    participant_type = case_when(
      participant_type == 1 ~ "Proband",
      participant_type == 2 ~ "ASD-Q",
      participant_type == 3 ~ "Sibling",
      participant_type == 4 ~ "Comparison",
      participant_type == 5 ~ "Mother",
      participant_type == 6 ~ "Father",
      TRUE ~ NA_character_
    )
  )

#### 2) Clean and parse date variables
#fhq_b1       = Child's DOB (date_dmy)
# acl_fhq_date = Date Family History Questionnaire completed (date_dmy)

AAB_data_recoded <- AAB_data_recoded %>%
  mutate(
    fhq_b1       = ymd(fhq_b1),
    acl_fhq_date = ymd(acl_fhq_date)
  )

# Derive age at questionnaire completion
AAB_data_recoded <- AAB_data_recoded %>%
  mutate(
    age_at_fhq = interval(fhq_b1, acl_fhq_date) / years(1)
  )

#### 3) Recode child's gender (fhq_b2)
AAB_data_recoded <- AAB_data_recoded %>%
  mutate(
    fhq_b2 = na_if(fhq_b2, 999),
    gender = case_when(
      fhq_b2 == 1 ~ "Male",
      fhq_b2 == 2 ~ "Female",
      TRUE ~ NA_character_
